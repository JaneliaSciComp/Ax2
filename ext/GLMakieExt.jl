...
@kwdef struct Observables
    ...
    nffts; noverlaps; nw; k; thresh; strelclose; strelopen; minpix;
...
function init()
    ...
    cb_jump_pref = @load_preference("cb_jmistakes", pref_defaults.cb_mistakes)
    ...
    hits = @lift begin
        fn = joinpath(datapath, string(splitext($(me_wav.selection))[1], "-hits.csv"))
        isfile(fn) ? readdlm(fn, ',', header=true)[1] : missing
    end

    misses = @lift begin
        fn = joinpath(datapath, string(splitext($(me_wav.selection))[1], "-misses.csv"))
        isfile(fn) ? readdlm(fn, ',', header=true)[1] : missing
    end

    false_alarms = @lift begin
        fn = joinpath(datapath, string(splitext($(me_wav.selection))[1], "-false-alarms.csv"))
        isfile(fn) ? readdlm(fn, ',', header=true)[1] : missing
    end
    ...
    thresh = @lift parse(Float64, $(tb_pval.stored_string))
    ...
    cb_mistakes = Checkbox(gl_zoom[1,5], checked = cb_jump_pref)
